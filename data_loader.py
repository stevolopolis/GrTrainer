...
class DataLoader:
    def __init__(self, path, batch_size, train_val_split=0.2):
        ...
        self.transformation = transforms.Compose([
            #transforms.RandomResizedCrop(params.OUTPUT_SIZE, scale=(.4, .7), ratio=(1, 1)),
    ...
    def load_grasp(self):
        for img_id_with_var in self.img_id_list:
            ...
            jitter_x = 0#random.uniform(-0.075, 0.075)
            jitter_y = 0#random.uniform(-0.075, 0.075)
...
angle = np.deg2rad(-degrees)
    ...
    grasp_list[:, :2] = np.squeeze((R @ (p.T-o.T) + o.T).T) / 1024
    if degrees == 0 or degrees == 180:
        grasp_list[:, 2] = grasp_list[:, 2] * 180 - 90
    elif degrees == 90 or degrees == -90:
        cond = grasp_list[:, 2] < 0.5
        grasp_list[:, 2] = np.where(cond, grasp_list[:, 2] * 180, grasp_list[:, 2] * 180 - 180)
